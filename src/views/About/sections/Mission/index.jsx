...
const missions = [
  {
    ...
    body: "به اکوسیستم زنده‌ای تبدیل شده که توش نوجوونا فرصت تجربه‌کردن و شکست خوردن رو داشته باشن و بر اساس استعدادها و توانمندی‌هاشون توی مسیر کارآفرینی هدایت بشن.",
    rotation: "rotate-[1.5deg]",
  },
  {
    theme: "teal",
    title: "پرورش نوجوونای ارزش‌آفرین",
    body: "توی رکاد به نوجوونا کمک می‌کنیم آینده روشن‌تری رو بسازن؛ توی دنیای نوجوونی وارد اکوسیستم استارتاپی بشن و ارتباطاتشون رو تقویت کنن.",
    rotation: "-rotate-[1.5deg]",
  },
];

const THEME_MAP = {
  magenta: { bg: "bg-magenta", border: "border-magenta", titleColor: "text-white", bodyColor: "text-white/85" },
  teal: { bg: "bg-teal", border: "border-teal", titleColor: "text-white", bodyColor: "text-white/85" },
};
          ...
          {missions.map((m) => {
            ...
            return (
              <div key={m.title} className={`relative ${m.rotation} transition-all duration-500 hover:rotate-0 hover:-translate-y-1`}>
                <div className={`absolute top-[0.25rem] left-[0.25rem] w-full h-full ${t.bg} rounded-[1.5rem] sm:rounded-[2.5rem] lg:rounded-[3rem] [corner-shape:squircle] opacity-50`} />
                <div className={`relative z-10 ${t.bg} border-2 ${t.border} rounded-[1.5rem] sm:rounded-[2.5rem] lg:rounded-[3rem] [corner-shape:squircle] p-6 sm:p-8 lg:p-10 min-h-[16rem] sm:min-h-[18rem] lg:min-h-[20rem] flex flex-col justify-between overflow-hidden`}>
                  <div className="absolute inset-0 pointer-events-none overflow-hidden">
                    <img src={patternBg} alt="" aria-hidden="true" className="w-full h-full object-cover opacity-10 mix-blend-overlay" />
                  </div>
                  <div className="relative z-10 flex flex-col gap-4 sm:gap-5">
                    <h3 className={`font-black text-[1.25rem] sm:text-[1.625rem] lg:text-[1.875rem] leading-[1.3] ${t.titleColor}`}>{m.title}</h3>
                    <p className={`text-[0.875rem] sm:text-[1rem] leading-[1.8] ${t.bodyColor}`}>{m.body}</p>
                  </div>
                </div>
              </div>
            );
