...
function EcoCard({ title, body, featured, tilt, isActive, cardRef }) {
  return (
    <article
      ...
      className={`
        group p-3 xs:p-4 sm:p-6 flex flex-col gap-2 sm:gap-4 min-h-[8rem] sm:min-h-[10rem]
        ...
        border-[0.11875rem] border-primary hover:border-white
        rounded-[0_1.90875rem_0_1.90875rem] [corner-shape:squircle]

        ${isActive ? "-translate-y-1.5 shadow-[0_15px_40px_-5px_rgba(89,187,175,0.25)] bg-primary border-white rotate-0" : ""}
      `}
    >
      {/* Icon */}
      <div
        className={`
          ...
          bg-primary group-hover:bg-male-darker
          ${isActive ? "bg-male-darker" : ""}
        `}
      >
        <span
          className={`w-3.5 h-3.5 xs:w-4 xs:h-4 sm:w-5 sm:h-5 text-male-darker group-hover:text-white transition-colors duration-300 ${
            isActive ? "text-white" : ""
          }`}
        >
          <UserIcon />
...
export default function Ecosystem() {
  ...
  return (
      <section
        className="py-[6.25rem] sm:py-[6.875rem] lg:py-[8.125rem] px-4 sm:px-6 relative overflow-hidden max-h-[calc(100vh-2rem)]"
        style={{
          background:
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-0 overflow-hidden z-0">
          <div className="absolute -top-20 -right-20 w-[26.25rem] h-[26.25rem] rounded-full bg-primary/10 blur-[5.625rem]" />
          <div className="absolute bottom-0 left-0 w-[18.75rem] h-[18.75rem] rounded-full bg-male/40 blur-[4.375rem]" />
        </div>
        ...
        <Container className="relative z-10">
          ...
          <h2 className="text-center font-black text-[1.375rem] xs:text-[1.5rem] sm:text-[2.375rem] lg:text-[2.875rem] leading-[1.3] text-white mb-3 sm:mb-4 flex flex-wrap justify-center items-center gap-x-2">
            ...
            <span className="inline-block rotate-[1.9deg] text-primary">
              اکوسیستم
            </span>
